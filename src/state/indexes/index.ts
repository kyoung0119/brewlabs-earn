...
export const fetchIndexesUserDataAsync = (account: string, chainId: ChainId) => async (dispatch, getState) => {
  ...
  fetchUserIndexNftData(account, chainId, indexes.filter((index) => !index.isFinished)[0]?.indexNft).then((nftInfo) =>
    dispatch(
      setIndexesUserData(
...
export const updateUserIndexNftInfo = (account: string, chainId: ChainId) => async (dispatch, getState) => {
  ...
  const nftInfo = await fetchUserIndexNftData(account, chainId, indexes.filter((index) => !index.isFinished)[0]?.indexNft);
  dispatch(
    setIndexesUserData(
      indexes.map((pool) => ({
        pid: pool.pid,
