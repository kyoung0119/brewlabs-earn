...
import DEFAULT_TOKEN_LIST from 'config/constants/tokenLists/default.tokenlist.json'
import APESWAP_TOKEN_LIST from 'config/constants/tokenLists/apeswap.json'
...
export function listToTokenMap(list: TokenList): TokenAddressMap {
  ...
  const map = list.tokens.reduce<TokenAddressMap>(
    (tokenMap, tokenInfo) => {
      const checkedTokenInfo = { ...tokenInfo, address: getAddress(tokenInfo.address) }
      const tags: TagInfo[] =
        checkedTokenInfo.tags
          ?.map((tagId) => {
            if (!list.tags?.[tagId]) return undefined
            return { ...list.tags[tagId], id: tagId }
          })
          ?.filter((x): x is TagInfo => Boolean(x)) ?? []
      const token = new WrappedTokenInfo(checkedTokenInfo, tags)
...
function combineMaps(map1: TokenAddressMap, map2: TokenAddressMap): TokenAddressMap {
  return {
    ...
    [ChainId.CRONOS]: { ...map1[ChainId.CRONOS], ...map2[ChainId.CRONOS] },
...
export function useCombinedActiveList(): TokenAddressMap {
  ...
  const apeTokenMap = listToTokenMap(APESWAP_TOKEN_LIST)
  return combineMaps(combineMaps(activeTokens, defaultTokenMap), apeTokenMap)
}
